...
def main():
    ...
    if run_case_types == ["Excel"]:  # 仅运行Excel驱动的general case
        ...
        for folder in run_case_folders:
            ...
            discover = unittest.defaultTestLoader.discover(os.path.abspath(path),
                                                           ...
                                                           pattern='test_*.py',
                                                           top_level_dir=os.path.abspath(get_tc_rootdir()))
    ...
    if args.report:
        ...
        report_file_path = r'../testreport/report-%s-%s.html' % (args.config_name, now)
    # report_file_path = r'../testreport/report.html' # 测试Jenkins publish report功能
    ...
    if args.email:
        SendEmail().send_main(report_file_path, "%s test %s测试报告" % (test_type, args.config_name), args.email)
