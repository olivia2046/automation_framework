...
class RuleParser(object):
    ...
    class Functions(object):
        ...
        def eq(self, *args):
            ...
            if isinstance(args[1],str) and args[1].upper()=='TRUE':
                ...
                if result is False:
                    logging.info("%s is not True"%args[0])
                return result
            elif isinstance(args[1],str) and args[1].upper()=='FALSE':
                result = (args[0] is False)
                if result is False:
                    logging.info("%s is not False"%args[0])
                return result
            elif isinstance(args[0],list) and isinstance(args[1],list):
                result =  my_list_cmp(args[0],args[1])
                if result is False:
                    logging.info("%s not equal to %s"%(args[0],args[1]))
                return result
        ...
        def neq(self, *args):
            result = (args[0] != args[1])
            if result is False:
                logging.info("%s equal to %s"%(args[0],args[1]))
            return result
        ...
        def in_(self, *args):
            ...
            if result is False:
                logging.info("%s not in %s"%(args[0],args[1]))
            return result
        ...
        def not_in_(self, *args):
            ...
            if result is False:
                logging.info("%s in %s" % (args[0], args[1]))
            return result

        def gt(self, *args):
            result = (args[0] > args[1])
            if result is False:
                logging.info("%s not greater than %s" % (args[0], args[1]))
            return result

        def gte(self, *args):
            result = (args[0] >= args[1])
            if result is False:
                logging.info("%s not greater than and not equal to %s" % (args[0], args[1]))
            return result

        def lt(self, *args):
            result = args[0] < args[1]
            if result is False:
                logging.info("%s not less than %s" % (args[0], args[1]))
            return result

        def lte(self, *args):
            result = (args[0] <= args[1])
            if result is False:
                logging.info("%s not less than and not equal to %s" % (args[0], args[1]))
            return result

        def not_(self, *args):
            result = (not args[0])
            if result is False:
                logging.info("%s is %s" % (args[0], args[1]))
            return result
        ...
        def contains_(self, *args):
            ...
            if result is False:
                logging.info("%s not contains %s" % (args[0], args[1]))
            return result
        ...
        def not_contains_(self, *args):
            ...
            if result is False:
                logging.info("%s contains %s" % (args[0], args[1]))
            return result

        def set_(self,*args):
            return set(args[0])

        def distinct_(self,*args):
        ...
        def like_(self, *args):
            ...
            if args[1]=='None':
                ...
                args_1 = args[1].replace("%", ".*?")
            if isinstance(args[0], list):
                for arg in args[0]:
                    result = re.search(args_1, arg)
                    if result is None:
                        result = False
                        logging.info("%s not like %s" % (arg, args[1]))
                    else:
                        result = True
    ...
    def _evaluate(self, rule, fns, f_list_result):
        ...
        r = []
        #if rule==[]: #
        if len(rule)<2: #如["=","$.message",[]]的最后一个元素,或单纯一个元素的列表
            return rule
        for item in rule:
            if len(r)>0 and (r[0]=='and' or r[0]=='or'):
                ...
                f_list_result=0
            item_eval = _recurse_eval(item,f_list_result)
            if len(r)>0 and r[0]=="or" and item_eval is True: # or表达式中遇到True即返回
                return True
            if len(r)>0 and r[0]=="and" and item_eval is False: # and表达式中遇到False即返回
                return False
        ...
        if len(r)>1 and r[0]!="and_" and r[0]!="or_" and type(r[1]) is str and (r[1].startswith('$.') or r[1].startswith('$[')): # and, or连接的表达式的第一个参数不会是jsonpath
