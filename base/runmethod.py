...
class RunMethod:
    ...
    #def post_main(self, url, json, headers=None, verify=False, new_session=False):
        ...
        if headers is None:
            # 部分项目必须使用data参数, 否则post请求不成功, 也有部分项目post json格式的数据必须使用json参数
            res = s.post(url=url, data=data, json=json, verify=verify, cert = cert, timeout=self.TIMEOUT)
        else:

            res = s.post(url=url, data=data, json=json, headers=headers, verify=verify, cert = cert, timeout=self.TIMEOUT)
    ...
    def get_main(self, url, data=None, json=None, headers=None, verify=None, cert = None, new_session=False):
        ...
        if new_session:
            ...
            s = glo.get_value('Session') #获取全局变量Session
        if headers is None:
            res = s.get(url=url, params=data, json=json, verify=verify, cert=cert, timeout=self.TIMEOUT)

        else:
            res = s.get(url=url, params=data,  json=json, headers=headers, verify=verify,cert = cert, timeout=self.TIMEOUT)
    ...
    def delete_main(self, url, data=None, json=None, headers=None, verify=False, cert = None, new_session=False):
        ...
        if new_session:
            ...
            s = glo.get_value('Session') #获取全局变量Session
        if headers is None:
            res = s.delete(url=url, params=data, json=json, verify=verify, cert = cert, timeout=self.TIMEOUT)

        else:
            res = s.delete(url=url, params=data,  json=json, headers=headers, verify=verify, cert = cert, timeout=self.TIMEOUT)
    ...
    def put_main(self, url, data=None, json=None, headers=None, verify=False, cert = None, new_session=False):
        ...
        if new_session:
            ...
            s = glo.get_value('Session') #获取全局变量Session
        if headers is None:
            res = s.put(url=url, params=data, json=json, verify=verify, cert = cert, timeout=self.TIMEOUT)

        else:
            res = s.put(url=url, params=data,  json=json, headers=headers, verify=verify, cert = cert, timeout=self.TIMEOUT)
