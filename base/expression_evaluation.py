...
def eval_from_string(target_str):
    ...
    try:
        ...
        #elif isinstance(target_str,dict):
        else:
        ...
        #if isinstance(target_str,str): #json.loads非json格式的字符串，结果仍为字符串
        for k,v in target_dict.items():
            #if isinstance(v,str) and re.match(r'.*\${.+?}.*',v): #值为表达式
            if re.match(r'.*\${.+?}.*', str(v)):  # 值为表达式
                target_dict[k]=eval_from_string(v)

        return target_dict
        # else: # else分支应走和exception相同的处理，逻辑结构不合理
        #     return_str = True
        ...
        if return_str == True:
            if re.match(r".*\(.+\)$",target_str):#对于形如‘+1#_#BA005006’的字符串（某项目的标签字段），eval也能成功并且返回为1，
                                                #因此限定带括号的表达式才做eval
                try:
                ...
                return target_str
            #对于形如‘+1#_#BA005006’的字符串（某项目的标签字段），eval也能成功并且返回为1，因此需要再加上str()调用，增加了复杂度
