...
def get_certfile_path():
    """ get certificate file path used for api test where required
...
def get_verify_cert():
    try:
        ...
        if verify.upper() == 'FALSE':
            ...
            verify = True
            cert_file_path = get_certfile_path()
            if cert_file_path is None or cert_file_path =='': # no need for certification file
                cert = None
            else:
                #cert = sys.path[0] + '/../' + cert_file_path
                cert = sys.path[0] + os.sep + cert_file_path
        return verify, cert
    except Exception as e:
