    ...
    def execute(self,casedata):
        ...
        if '请求数据' in casedata.keys() and casedata['请求数据']!='':
            ...
            try:
                ...
                elif request_data.startswith('data_label:'): #request_data为data文件中标签名
                    jutil = JsonUtil(get_data_file())
                    data = jutil.get_data(request_data) #json格式数据
                    data_str = repr(data)

                elif '=' in request_data: #形如url=/xxx&type=POST&params={"id":"123","type":"1","paths":{}}&contentType=1的接口
